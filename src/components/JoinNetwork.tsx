  ...
  boxesData: [
    {
      ...
      description:
        'We are building a network where participants can facilitate knowledge sharing and evolve the insurance industry.',
      image: cube,
    },
    {
      title: 'A Professional Network',
      description:
        'The Fida insurance network connects professionals in the industry including brokers, underwriters and capacity providers to reduce unnecessary friction in the execution and trading of insurance contracts.',
      image: circles,
    },
  ],
};
...
export const JoinNetwork = () => {
  ...
  return (
    <Wrapper isChangedMargin={isTablet}>
      <TitleWrapper isChangedMargin={isTablet} ref={elementOneRef} isOnScreen={isElementOneVisible}>
        <Text size={isMobile ? 3 : 5} label={text.title.join} toUpperCase fontWeight={700} />
        <Text
          size={isMobile ? 3 : 5}
          label={text.title.our}
      ...
      >
        {text.boxesData.map((boxData) => (
          <GradientBox key={boxData.title} boxData={boxData} />
        ))}
      </StyledBoxesWrapper>
    </Wrapper>
  );
